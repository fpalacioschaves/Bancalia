...
<body class="bg-slate-50 text-slate-900">
  <main class="mx-auto max-w-md px-4 py-10">
    <div class="mb-6 flex items-center justify-center gap-2">
      <span
        class="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-slate-900 text-white text-lg font-bold">B</span>
    ...
    <?php if ($flashError || $errors): ?>
      <div class="mb-4 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800">
        <?php if ($flashError): ?>
          <div>• <?= h($flashError) ?></div><?php endif; ?>
        <?php foreach ($errors as $e): ?>
          <div>• <?= h($e) ?></div><?php endforeach; ?>
      </div>
    <?php endif; ?>
    <?php if ($flashOk): ?>
      <div class="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800"><?= h($flashOk) ?>
      </div>
    <?php endif; ?>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <form method="post" class="space-y-4" autocomplete="off">
        ...
        <div class="flex items-center justify-between">
          <a href="<?= h(PUBLIC_URL) ?>/auth/register.php" class="text-sm text-slate-600 hover:underline">Crear
            cuenta</a>
          <button
            class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">Entrar</button>
        </div>
      </form>
    </div>
  </main>
