...
focus:ring-2 focus:ring-slate-400 focus:border-slate-400" aria-label="Filtrar por estado">
    <option value="">Todos los estados</option>
    <option value="borrador" <?= $estado === 'borrador' ? 'selected' : '' ?>>Borrador</option>
    <option value="publicado" <?= $estado === 'publicado' ? 'selected' : '' ?>>Publicado</option>
  </select>

  <select name="tipo" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:outline-none
focus:ring-2 focus:ring-slate-400 focus:border-slate-400" aria-label="Filtrar por tipo">
  ...
  <a href="<?= PUBLIC_URL ?>/admin/examenes/create.php"
...
<?php else: ?>
  <div class="overflow-x-auto rounded-lg border border-slate-200 bg-white">
    <table id="exTable" class="min-w-full divide-y divide-slate-200 text-sm">
          ...
          <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-sm">
              <?php if (($r['tipo'] ?? 'examen') === 'practica'): ?>
                <span
                  class="inline-flex items-center rounded-full bg-sky-50 px-2 py-0.5 text-[12px] font-medium text-sky-700 ring-1 ring-sky-200">
                  Hoja de actividades
                </span>
              <?php else: ?>
                <span
                  class="inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-[12px] font-medium text-indigo-700 ring-1 ring-indigo-200">
                  Examen
                </span>
              <?php endif; ?>
            </td>
            <td class="px-4 py-3 text-sm">
              <?php if ($r['estado'] === 'publicado'): ?>
                <span
                  class="inline-flex items-center rounded-full bg-emerald-50 px-2 py-0.5 text-[12px] font-medium text-emerald-700 ring-1 ring-emerald-200">
                  Publicado
                </span>
              <?php else: ?>
                <span
                  class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[12px] font-medium text-slate-700 ring-1 ring-slate-200">
                  Borrador
                </span>
              <?php endif; ?>
            </td>
            ...
            <td class="px-4 py-3 text-sm">
              <div class="flex justify-end gap-2 flex-wrap">
                <a href="<?= PUBLIC_URL ?>/admin/examenes/edit.php?id=<?= (int) $r['id'] ?>" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm
 font-medium text-slate-700 hover:bg-slate-100">Editar</a>

                <a href="<?= PUBLIC_URL ?>/admin/examenes/actividades.php?id=<?= (int) $r['id'] ?>" class="inline-flex items-center rounded-md border border-indigo-300 px-2 py-1 text-xs font-medium
 text-indigo-700 hover:bg-indigo-50">
                  Actividades
                </a>

                <a href="<?= PUBLIC_URL ?>/admin/examenes/preview.php?id=<?= (int) $r['id'] ?>" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs
 font-medium text-slate-700 hover:bg-slate-100">
                  Vista previa
                </a>

                <!-- Botón: INTENTOS -->
                <a href="<?= PUBLIC_URL ?>/admin/examenes/intentos.php?examen_id=<?= (int) $r['id'] ?>" class="inline-flex items-center rounded-lg border border-fuchsia-200 bg-fuchsia-50 px-3 py-1.5
 text-xs font-medium text-fuchsia-700 hover:bg-fuchsia-100">
                ...
                <form method="post" action="<?= PUBLIC_URL ?>/admin/examenes/delete.php"
                  ...
                  <button type="submit" class="inline-flex items-center rounded-lg bg-rose-600 px-3 py-1.5 text-sm font-semibold text-white
 hover:bg-rose-500">Eliminar</button>
                </form>
              </div>
            </td>
          </tr>
