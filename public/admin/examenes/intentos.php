...
<p class="text-sm text-slate-500 mb-6">
  Examen: <span class="font-semibold"><?= h($examen['titulo']) ?>span>
</p>
...
<?php if (!$intentos): ?>
  <div
    class="mt-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center text-slate-500 text-sm">
        ...
        <?php foreach ($intentos as $it): ?>
          ...
          <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-sm">
              <?php if ($nota === null): ?>
                <span
                  class="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[12px] font-medium text-amber-700 ring-1 ring-amber-200">
                  Pendiente
                </span>
              <?php else: ?>
                <span
            ...
            <td class="px-4 py-3 text-sm">
              <div class="flex justify-end">
                <a href="<?= PUBLIC_URL ?>/admin/examenes/intento_ver.php?intento_id=<?= (int) $it['id'] ?>"
