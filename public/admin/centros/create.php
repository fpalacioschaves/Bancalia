...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    $centroService->create($_POST);

    flash('success', 'Centro creado correctamente.');
    if (($u['role'] ?? '') === 'profesor') {
    ...
    header('Location: ' . PUBLIC_URL . '/admin/centros/create.php');
    exit;
  }
}
...
<div class="grid gap-6 lg:grid-cols-3">
  <div class="lg:col-span-2 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
    <form method="post" action="" class="space-y-5" id="centroForm">
      ...
      <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        <button type="submit"
          class="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">Crear
          centro</button>
      </div>
    </form>
  </div>
  ...
  <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <p class="mt-2 text-xs text-slate-500">El marcador se calcula desde la dirección. Puedes arrastrarlo para afinar.
    </p>
  </div>
</div>
...
<script>
  ...
  async function geocode() {
    ...
    try {
      const r = await fetch('https://nominatim.openstreetmap.org/search?format=json&q=' + encodeURIComponent(q), { headers: { 'Accept-Language': 'es' } }); const d = await r.json();
