...
<div class="mb-6 flex items-center justify-between">
  ...
  <a href="<?= PUBLIC_URL ?>/admin/centros/create.php"
    class="inline-flex items-center rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">Nuevo
    centro</a>
</div>
...
<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
  <table id="idxTable" class="min-w-full divide-y divide-slate-200">
    ...
    <tbody class="divide-y divide-slate-200">
        ...
        <tr>
          ...
          <td class="px-3 py-2">
            <?php if ((int) $r['is_active'] === 1): ?>
              <span
                class="inline-flex rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800">Activo</span>
            <?php else: ?>
              <span
                class="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">Inactivo</span>
            <?php endif; ?>
          </td>
          <td class="px-3 py-2 text-right">
            <div class="flex justify-end gap-2">
              ...
              <form method="post" action="<?= PUBLIC_URL ?>/admin/centros/delete.php"
                onsubmit="return confirm('¿Seguro que quieres borrar este centro?');">
                <?= csrf_field() ?>
                <input type="hidden" name="id" value="<?= (int) $r['id'] ?>">
                <button
                  class="rounded-lg bg-rose-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-rose-500">Borrar</button>
              </form>
            </div>
          </td>
        </tr>
      ...
      <?php if (!$rows): ?>
        <tr>
          <td colspan="5" class="px-3 py-6 text-center text-sm text-slate-500">No hay centros.</td>
        </tr>
      <?php endif; ?>
    </tbody>
  </table>
