...
<div class="mb-6 flex items-center justify-between">
  ...
  <a href="<?= PUBLIC_URL ?>/admin/centros/index.php"
    class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Volver
    al listado</a>
</div>
...
<div class="grid gap-6 lg:grid-cols-3">
    ...
    <form method="post" action="" class="space-y-5" id="centroForm">
      ...
      <div class="grid gap-4 sm:grid-cols-[2fr,1fr]">
        <div>
          ...
          <input name="nombre" id="nombre" type="text" required value="<?= h($centro['nombre']) ?>"
        ...
        <div>
          ...
          <input name="slug" id="slug" type="text" value="<?= h($centro['slug']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
      </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input name="direccion" id="direccion" type="text" placeholder="Calle, número"
            value="<?= h((string) $centro['direccion']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-700">Código postal</label>
          <input name="cp" id="cp" type="text" placeholder="28001" value="<?= h((string) $centro['cp']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
      </div>
      ...
      <div class="grid gap-4 sm:grid-cols-3">
        <div>
          ...
          <input name="localidad" id="localidad" type="text" value="<?= h((string) $centro['localidad']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-700">Provincia</label>
          <input name="provincia" id="provincia" type="text" value="<?= h((string) $centro['provincia']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-700">Comunidad</label>
          <input name="comunidad" id="comunidad" type="text" value="<?= h((string) $centro['comunidad']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
      </div>
      ...
      <div class="grid gap-4 sm:grid-cols-3">
        <div>
          ...
          <input name="telefono" type="text" placeholder="+34 600 000 000" value="<?= h((string) $centro['telefono']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-700">Email</label>
          <input name="email" type="email" placeholder="centro@dominio.es" value="<?= h((string) $centro['email']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-700">Web</label>
          <input name="web" type="text" placeholder="https://…" value="<?= h((string) $centro['web']) ?>"
            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-slate-400">
        </div>
      </div>

      <input type="hidden" name="lat" id="lat" value="<?= h((string) $centro['lat']) ?>">
      <input type="hidden" name="lng" id="lng" value="<?= h((string) $centro['lng']) ?>">
      ...
      <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        <button type="submit"
          class="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">Guardar
          cambios</button>
      </div>
    </form>
  </div>
  ...
  <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <p class="mt-2 text-xs text-slate-500">El marcador se actualiza desde la dirección. Puedes arrastrarlo para afinar.
    </p>
  </div>
</div>
