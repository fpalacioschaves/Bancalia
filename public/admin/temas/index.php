...
<form method="get" action=""
  class="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-[1fr,220px,220px,260px,auto,auto] items-stretch">
  <input type="search" name="q" value="<?= h($q) ?>" placeholder="Buscar por tema/asignatura/curso/familia…"
...
<?php if (!$rows): ?>
  <div class="rounded-lg border border-slate-200 bg-white p-6 text-slate-600">
    ...
    <a href="<?= PUBLIC_URL ?>/admin/temas/create.php" class="text-indigo-600 hover:underline font-medium">Crea el
      primero</a>.
  </div>
<?php else: ?>
      ...
      <tbody class="divide-y divide-slate-200">
        <?php foreach ($rows as $r): ?>
          <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-sm text-slate-800"><?= h($r['asignatura']) ?></td>
            ...
            <td class="px-4 py-3 text-sm">
              <div class="flex justify-end gap-2">
                ...
                <form method="post" action="<?= PUBLIC_URL ?>/admin/temas/delete.php"
