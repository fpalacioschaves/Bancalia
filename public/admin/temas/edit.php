...
<div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
  <form method="post" action="" class="space-y-5">
    ...
    <div class="grid gap-4 sm:grid-cols-3">
      ...
      <div>
        <label for="asignatura_id" class="mb-1 block text-sm font-medium text-slate-700">Asignatura <span
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      <div>
        <label for="nombre" class="mb-1 block text-sm font-medium text-slate-700">Nombre <span
            class="text-rose-600">*</span></label>
        <input id="nombre" name="nombre" type="text" required value="<?= h($row['nombre']) ?>"
      ...
      <div>
        ...
        <input id="slug" name="slug" type="text" value="<?= h($row['slug']) ?>"
          class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-slate-400">
    ...
    <div class="grid gap-4 sm:grid-cols-3">
      <div>
        <label for="numero" class="mb-1 block text-sm font-medium text-slate-700">Número <span
            class="text-rose-600">*</span></label>
        <input id="numero" name="numero" type="number" min="1" value="<?= (int) $row['numero'] ?>"
      ...
      <div class="sm:col-span-2">
        ...
        <textarea id="descripcion" name="descripcion" rows="3"
          class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-slate-400"><?= h((string) $row['descripcion']) ?></textarea>
      </div>
    </div>

    <div class="flex items-center gap-2">
      <input id="is_active" name="is_active" type="checkbox"
        class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-400" <?= (int) $row['is_active'] === 1 ? 'checked' : '' ?>>
...
<script>
  ...
  $byFam = [];
  foreach ($cursos as $c) {
    $byFam[$c['familia_id']][] = ['id' => $c['id'], 'nombre' => $c['nombre']];
  }
  ...
  $byCurso = [];
  foreach ($asigs as $a) {
    $byCurso[$a['curso_id']][] = ['id' => $a['id'], 'nombre' => $a['nombre']];
  }
