...
<form method="get" action="" class="mb-5 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
  ...
  <input id="q" type="search" name="q" value="<?= h($q) ?>" placeholder="Buscar por nombre o slug"
  ...
  <a href="<?= PUBLIC_URL ?>/admin/familias/create.php"
...
<?php if (!$rows): ?>
  <div class="rounded-lg border border-slate-200 bg-white p-6 text-slate-600">
    ...
    <a href="<?= PUBLIC_URL ?>/admin/familias/create.php" class="text-indigo-600 hover:underline font-medium">Crea la
      primera</a>.
  </div>
<?php else: ?>
  <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <table class="min-w-full divide-y divide-slate-200">
      <thead class="bg-slate-50">
        <tr>
          <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">Nombre
          </th>
          <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">Slug
          </th>
          <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">Activa
          </th>
          <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">
            Actualizado</th>
          <th scope="col" class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-600">
            Acciones</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-slate-200">
        <?php foreach ($rows as $r): ?>
          <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-sm text-slate-700"><code
                class="rounded bg-slate-100 px-1.5 py-0.5 text-[11px]"><?= h($r['slug']) ?></code></td>
            ...
            <td class="px-4 py-3 text-sm text-slate-600"><?= h($r['updated_at']) ?></td>
            <td class="px-4 py-3 text-sm">
              <div class="flex justify-end gap-2">
                ...
                <form method="post" action="<?= PUBLIC_URL ?>/admin/familias/delete.php"
